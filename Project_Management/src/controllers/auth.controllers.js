...
const login = asyncHandler(async (req, res) => {
  ...
  if (!user) {
    throw new Apierrors(400, "User doesn't exixts");
  }

  const isPasswordValid = await user.isPasswordCorrect(password);

  if (!isPasswordValid) {
    throw new Apierrors(400, "Credentials is incorrect")
  }
...
const resendEmailVerification = asyncHandler(async(req, res) => {
  const user = User.findById(res.user?._id)
...
const forgotPasswordRequest = asyncHandler(async(req, res) => {
  ...
  await sendEmail({
    ...
    mailgenContent: emailVerificationMailgenContent(
      user.username,
      `${process.env.FORGOT_PASSWORD_REDIRECT_RUL}/${unHashedToken}`,
    ),
  });
...
const resetForgotPassword = asyncHandler(async(req,res) => {
  ...
  if(!user){
    throw new Apierrors(409, "Token is invalid or expired")
  }
